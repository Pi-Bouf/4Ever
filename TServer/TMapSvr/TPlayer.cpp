	...
	case VICTORY_MONTH_TITLE:
		{
			...
			DWORD dwLastVic = NULL;
			WORD wLastTitleID = NULL;
			BYTE bSet = FALSE;
			BYTE bFound = FALSE;

			for(itTITLE = _AtlModule.m_mapTTITLE.begin(); itTITLE != _AtlModule.m_mapTTITLE.end(); itTITLE++)
			{
				if((*itTITLE).second->m_bKind == VICTORY_MONTH_TITLE && dwValue >= (*itTITLE).second->m_dwRequirement)
				{
					itTITLE++;
					if(dwValue < (*itTITLE).second->m_dwRequirement)
					{
						itTITLE--;
					...
					itTITLE--;
					dwLastVic = (*itTITLE).second->m_dwRequirement;
					wLastTitleID = (*itTITLE).second->m_wTitleID;

					if((*itTITLE).second->m_wTitleID == m_wTitleID)
						bFound = TRUE;
				}
			}
			if(dwValue >= dwLastVic && !bSet)
			{
				vTitleID.push_back(wLastTitleID);
				bSet = TRUE;
			}
	...
	case DEFEATS_MONTH_TITLE:
		{
			...
			for(itTITLE = _AtlModule.m_mapTTITLE.begin(); itTITLE != _AtlModule.m_mapTTITLE.end(); itTITLE++)
			{
				if((*itTITLE).second->m_bKind == DEFEATS_MONTH_TITLE && dwValue >= (*itTITLE).second->m_dwRequirement)
				{
					itTITLE++;
					if(dwValue < (*itTITLE).second->m_dwRequirement)
					{
					...
					itTITLE--;
					dwLastDef = (*itTITLE).second->m_dwRequirement;
					wLastTitleID = (*itTITLE).second->m_wTitleID;
	...
	case DEATH_MONTH_TITLE:
		{
			...
			for(itTITLE = _AtlModule.m_mapTTITLE.begin(); itTITLE != _AtlModule.m_mapTTITLE.end(); itTITLE++)
			{
				if((*itTITLE).second->m_bKind == DEATH_MONTH_TITLE && dwValue >= (*itTITLE).second->m_dwRequirement)
				{
					itTITLE++;
					if(dwValue < (*itTITLE).second->m_dwRequirement)
					{
					...
					itTITLE--;
					dwLastDeath = (*itTITLE).second->m_dwRequirement;
					wLastTitleID = (*itTITLE).second->m_wTitleID;
		...
		break;
	case CUSTOM_TITLE: break;
	case HONOUR_TITLE:
		{
			DWORD dwLastHonour = NULL;
			WORD wLastTitleID = NULL;
			BYTE bSet = FALSE;
			BYTE bFound = FALSE;
			...
			for(itTITLE = _AtlModule.m_mapTTITLE.begin(); itTITLE != _AtlModule.m_mapTTITLE.end(); itTITLE++)
			{
				if((*itTITLE).second->m_bKind == HONOUR_TITLE && dwValue >= (*itTITLE).second->m_dwRequirement)
				{
					itTITLE++;
					if(dwValue < (*itTITLE).second->m_dwRequirement)
					{
					...
					itTITLE--;
					dwLastHonour = (*itTITLE).second->m_dwRequirement;
					wLastTitleID = (*itTITLE).second->m_wTitleID;
	...
	case GOLD_TITLE:
		{
			...
			for(itTITLE = _AtlModule.m_mapTTITLE.begin(); itTITLE != _AtlModule.m_mapTTITLE.end(); itTITLE++)
			{
				if((*itTITLE).second->m_bKind == GOLD_TITLE && dwValue >= (*itTITLE).second->m_dwRequirement)
				{
					itTITLE++;
					if(dwValue < (*itTITLE).second->m_dwRequirement)
					{
					...
					itTITLE--;
					dwLastGold = (*itTITLE).second->m_dwRequirement;
					wLastTitleID = (*itTITLE).second->m_wTitleID;
