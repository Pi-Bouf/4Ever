...
CTClientChar* CTClientPet::TakeDown()
{
	...
	m_pTakeUpChar->m_wMoveDIR = MOVE_NONE;
...
void CTClientPet::SetSpeedWhenRiding(WORD wMulti)
{
	...
	{
		m_fBaseSpeedFactor = m_fBaseSpeedFactor_org;
		m_fBaseSpeedFactor_org = 0;

		D3DXVECTOR3 vDIR( 0.0f, 0.0f, -(TDEF_SPEED * 10));
		D3DXMATRIX vROT;
	...
	{
		m_fBaseSpeedFactor_org = m_fBaseSpeedFactor;
		m_fBaseSpeedFactor = m_fTDEFAULTSPEED + FLOAT(125) / 100.0f;

		D3DXVECTOR3 vDIR( 0.0f, 0.0f, -((TDEF_SPEED * 10) + m_fBaseSpeedFactor));
		D3DXMATRIX vROT;
