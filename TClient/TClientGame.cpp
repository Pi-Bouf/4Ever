...
{
	bool operator () ( CTClientObjBase * _Left, CTClientObjBase * _Right) const
	{
...
void CTClientGame::ResetTFOG( BYTE bWATER)
{
	...
	if(pTFOG)
		for( int i=0; i<BYTE(pTFOG->m_vTLOCAL.size()); i++)
		{
...
void CTClientGame::ResetVisibleOBJ( DWORD dwTick)
{
	...
	if( pTPANNEL && pTPANNEL->m_bShowPath && pTPANNEL->m_dwCurQuestID )
		for( int i=TNODEGUIDE_COUNT - 1; i>=0; i--)
...
BYTE CTClientGame::ResetTPATHDATA()
{
	...
	if( pTPANNEL && pTPANNEL->m_dwCurQuestID )
	{
		...
		if( pTQUEST && pTQUEST->m_pTQUEST )
		{
			...
			if(!pTNODEPATH)
				for( int i=0; i<INT(pTQUEST->m_pTQUEST->m_vTTERMPOS.size()); i++)
...
#ifdef NEW_IF
		CTGaugePannel* pTGaugePannel = static_cast<CTGaugePannel*>(m_vTFRAME[TFRAME_GAUGE]);
		pTGaugePannel->m_pRegionNew->m_strText = m_pMainChar->m_pTREGION->m_strNAME;
#else
		...
		for( BYTE i=0; i<TRECALLAIACT_COUNT; i++)
		{
			if(pUI->m_pTRECALLAI_NEW[i]->HitTest(pt) && pUI->m_pTRECALLAI_NEW[i]->IsVisible())
			{
				if(i == TRECALLAIACT_STATUS)
				{
					...
					pUI->m_pTRECALLAIDOWN_NEW[i]->SetCurImage(nCurImage);
				}
		...
		case TFRAME_CINEMATIC	:
			{
				...
				if(CTClientGame::m_vTOPTION.m_bAutoHelp)
					pGauge->NotifyFrameStaticMoveReset(TRUE);
				else
				{
					pGauge->m_bCanClickReset = TRUE;
					pGauge->NotifyFrameStaticMove(TRUE);
				}
#endif
			}
...
void CTClientGame::CheckMainUI()
{
	...
	{
		if( pUI->m_pTRECALLAIDOWN_NEW[i]->IsVisible() )
		{
			if(CanControl())
			{
				if(i == TRECALLAIACT_STATUS)
...
void CTClientGame::CalcDynamicHelp( DWORD dwTick)
{
	...
	if( !m_vTOPTION.m_bAutoHelp && m_bStartGame)
	{
		pTGaugePannel->m_pNotify->ShowComponent(FALSE);
		m_bStartGame = FALSE;
		pTGaugePannel->m_bCanClickReset = TRUE;
	}
	else if( !m_vTOPTION.m_bAutoHelp && pTGaugePannel->m_pNotify->IsVisible() )
	{
		pTGaugePannel->m_pNotify->ShowComponent(FALSE);
	...
	else if(m_bStartGame && m_vTOPTION.m_bAutoHelp)
	{
		pTGaugePannel->m_pNotify->ShowComponent(TRUE);
		pTGaugePannel->NotifyFrameStaticMove(TRUE);
...
void CTClientGame::OnActionCmdBtnDown(CTMiniPopupDlg* pPopupDlg, CTMiniPopupDlg::CTButtonMP* pButton)
{
	...
	if((pChar->m_wMoveDIR != MOVE_NONE || pChar->IsJump() || pChar->IsFall()) )
	{
		m_pChatFrame->ChatSysMSG(
			CTChart::LoadString( TSTR_INFO_TITLE),
			pChar->GetName(), 
