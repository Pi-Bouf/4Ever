	  ...
	  m_bNeedUpdate(TRUE), m_nSelectIdx(T_INVALID)
{
	...
	m_vItems[0].m_pMailImage = FindKid(ID_CTRLINST_MAIL_MAIL_NEW);
	m_vItems[0].m_pMailImage->GetComponentPos(&ptTemp);
	m_vItems[0].m_pMailImage->m_bNoHIT = TRUE;
	CPoint ptMailImageOffset = ptTemp - ptBase;

	m_vItems[0].m_pPackageImage = FindKid(ID_CTRLINST_MAIL_PACKAGE_NEW);
	m_vItems[0].m_pPackageImage->GetComponentPos(&ptTemp);
	m_vItems[0].m_pPackageImage->m_bNoHIT = TRUE;
	CPoint ptPackageImageOffset = ptTemp - ptBase;
	...
	for(INT i=1; i<MAX_LINE; ++i)
	{
		...
		m_vItems[i].m_pMailImage = new TComponent(this, m_vItems[0].m_pMailImage->m_pDESC);
		m_vItems[i].m_pPackageImage = new TComponent(this, m_vItems[0].m_pPackageImage->m_pDESC);
		...
		m_vItems[i].m_pMailImage->m_id = GetUniqueID();
		m_vItems[i].m_pPackageImage->m_id = GetUniqueID();
		...
		m_vItems[i].m_pMailImage->m_bNoHIT = TRUE;
		m_vItems[i].m_pPackageImage->m_bNoHIT = TRUE;

		//AddKid( m_vItems[i].m_pTypeTxt );
		AddKid( m_vItems[i].m_pSenderTxt );
		AddKid( m_vItems[i].m_pTitleTxt );
		AddKid( m_vItems[i].m_pTimeTxt );
		AddKid( m_vItems[i].m_pMailImage );
		AddKid( m_vItems[i].m_pPackageImage );
		...
		m_vItems[i].m_pMailImage->MoveComponent(ptBase+ptMailImageOffset);
		m_vItems[i].m_pPackageImage->MoveComponent(ptBase+ptPackageImageOffset);
	}
	...
	TComponent *pCheckBox = FindKid(ID_CTRLINST_CHECKBOX);
	RemoveKid(pCheckBox);
...
void CTMailListFrame::Item::ShowComponent(BOOL bShow)
{
	...
	m_pMailImage->ShowComponent(bShow);
	m_pPackageImage->ShowComponent(bShow);
}
// -------------------------------------------------------
void CTMailListFrame::Item::SetMail(LPTMAIL_SIMPLE pMail)
{
	...
	if( pMail )
	{
		...
		if(pMail->m_bType == 1)
			m_pMailImage->ShowComponent(FALSE);	
		else
			m_pPackageImage->ShowComponent(FALSE);
