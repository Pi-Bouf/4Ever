...
{
	{
		...
		{
			if( FALSE == (*itMAINTAIN)->m_pTSKILL->m_bStaticWhenDie || (*itMAINTAIN)->m_pTSKILL->m_bKind == WARRIOR_STATE)
...

	{
		...
		{
			if( FALSE == (*itMAINTAIN).second->m_pTSKILL->m_bStaticWhenDie || (*itMAINTAIN).second->m_pTSKILL->m_bKind == WARRIOR_STATE)
			{
				OnMaintainEnd(
...
void CTClientObjBase::ReleaseData()
{
	...
	for( int i=0; i<TNUMBERPOS_COUNT; i++)
		m_vDamageTick[i] = 0;
	m_vTDAMAGE.clear();
