...
BEGIN_MESSAGE_MAP( CMAPOBJBar, CBarContent)
	...
	ON_BN_CLICKED(IDC_GROUP, OnBnClickedGroup)
END_MESSAGE_MAP()
...
void CMAPOBJBar::EnableAllCtrl( BOOL bEnable)
{
	...
	if(m_nMode)
	{
		...
		m_cPosX.EnableWindow(bEnable && !pDoc->m_vSELOBJ.empty() && !m_nGroup);
		m_cPosY.EnableWindow(bEnable && !pDoc->m_vSELOBJ.empty() && !m_nGroup);
		m_cPosZ.EnableWindow(bEnable && !pDoc->m_vSELOBJ.empty() && !m_nGroup);
		m_cRotX.EnableWindow(bEnable && !pDoc->m_vSELOBJ.empty() && !m_nGroup);
		m_cRotY.EnableWindow(bEnable && !pDoc->m_vSELOBJ.empty());
		m_cRotZ.EnableWindow(bEnable && !pDoc->m_vSELOBJ.empty() && !m_nGroup);
	}
	else
	{
...
void CMAPOBJBar::OnBnClickedGroup()
{
	m_nGroup = m_cGroup.GetCheck();

	m_cRotX.EnableWindow( !m_nGroup );
	m_cRotZ.EnableWindow( !m_nGroup );
	m_cPosX.EnableWindow( !m_nGroup );
	m_cPosY.EnableWindow( !m_nGroup );
	m_cPosZ.EnableWindow( !m_nGroup );

	if( !m_cGroup.GetCheck() )
	{
		CMAPDoc *pDoc = (CMAPDoc *) m_pItem->GetMainDocument();
		pDoc->ClearSELOBJ();
		pDoc->RedrawAllView();
	...
	if(pDoc)
	{
		...
		for( int i=0; i<nCount; i++)
		{
			...
			if( m_nGroup )
			{
				...
				D3DXMATRIX vTempRY;
				D3DXMATRIX vTempRZ;
				D3DXMatrixIdentity( &vOffset );
				D3DXMatrixIdentity( &vTempT );
				D3DXMatrixIdentity( &vTempS );
				D3DXMatrixIdentity( &vTempRX );
				D3DXMatrixIdentity( &vTempRY );
				D3DXMatrixIdentity( &vTempRZ );
				...
				D3DXMatrixRotationX( &vTempRX, vROT.x );
				D3DXMatrixRotationY( &vTempRY, vROT.y );
				D3DXMatrixRotationZ( &vTempRZ, vROT.z );

				vTempT = vOffset * vTempS * vTempRX * vTempRY * vTempRZ;

				D3DXMATRIX vTrans;
				D3DXMatrixTranslation(
					&vTrans,
					vTempT._41 - vOffset._41,
					vTempT._42 - vOffset._42,
					vTempT._43 - vOffset._43 );

				pOBJ->m_vWorld = vRotX * vRotY * vRotZ * vTrans * vPos;
			}
			else
			{
