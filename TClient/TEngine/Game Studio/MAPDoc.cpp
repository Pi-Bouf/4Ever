	...
	for( int i=0; i<nCount; i++)
	{
		...
		if( pControl->m_nGroup )
		{
			...
			D3DXMatrixTranslation( 
				&vOffset,
				pMAPOBJ->m_vPOS.x + vPOS.x - vCENTER.x,
				pMAPOBJ->m_vPOS.y + vPOS.y - vCENTER.y,
				pMAPOBJ->m_vPOS.z + vPOS.z - vCENTER.z);
			...
			pMAPOBJ->m_vROT += vROT;
			pMAPOBJ->m_vPOS += D3DXVECTOR3( vTrans._41, vTrans._42, vTrans._43 );
		}
		else
		{
			pMAPOBJ->m_vSCALE += vSCALE;
