			...
			if( m_dwNodeCount > 0 )
			{
				...
				for( int i = 0; i<m_dwNodeCount; i++)
				{
					...
					if( m_pNode[nNODE].m_nPositionKeyCount > 0 )
					{
						...
						for( int j=0; j<m_pNode[nNODE].m_nPositionKeyCount; j++)
						{
					...
					if( m_pNode[nNODE].m_nRotationKeyCount > 0 )
					{
						...
						for( int j=0; j<m_pNode[nNODE].m_nRotationKeyCount; j++)
						{
							if( bTYPE == 1 )
							{
						...
						if( bTYPE != 1 )
							for( int j=m_pNode[nNODE].m_nRotationKeyCount - 1; j>0; j--)
							{
								D3DXQUATERNION vINV;
					...
					if( m_pNode[nNODE].m_nScaleKeyCount > 0 )
					{
						...
						for( int j=0; j<m_pNode[nNODE].m_nScaleKeyCount; j++)
						{
