...
BEGIN_MESSAGE_MAP(CDetailTextureDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_SAVETEX, OnBnClickedButtonSavetex)
END_MESSAGE_MAP()
...
BYTE CDetailTextureDlg::CanReplace( int nIndex)
{
	...
	for( int i=0; i<nCount; i++)
	{
		...
		if( pDETTEX && pDETTEX->m_dwTEXID != pDETTEX->m_dwREPID && pDETTEX->m_dwREPID == dwTexID )
			return FALSE;
	}
...
void CDetailTextureDlg::ResetTILE( LPBMPARAM lParam)
{
	...
	for( i=0; i<nCount; i++)
	{
		...
		mapINDEX.insert( MAPDWORD::value_type( i, wTILE));
	}
...
void CDetailTextureDlg::OnBnClickedButtonSavetex()
{
	CTShell dlg;
	dlg.m_strTitle = "Save to..";

	if( dlg.BrowseForFolder( GetSafeOwner() ) == IDOK )
	{
		CString strFileName;
		...
		{
			LPDETTEX pDETTEX = (LPDETTEX)pList->GetItemData( i );
			strFileName.Format( "%s\\%s_%d.jpg",
				dlg.m_strPath,
				pList->GetItemText( i, 0 ),
				(int)pDETTEX->m_fSCALE );

			CTextureItem* pItem = CItemBase::GetTextureItem( pDETTEX->m_dwTEXID );
			if( pItem )
			{
