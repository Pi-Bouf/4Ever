...
void CTachyonHUGEMAP::InitVB( CD3DDevice *pDevice)
{
	...
	for( int i = 0; i<m_nDiffuseCount; i++)
	{
								...
								LPIPOINT pCENTER)
{
	...
	if(m_bDrawShadow)
	{
		...
		for( int i = 0; i<INT(vUNIT.size()); i++)
		{
	...
	if(!pTILE->empty())
	{
		...
		for( int i = 0; i<m_nBlockCount; i++)
		{
			...
			if( nWIDTH > 1 || bREGION != BLANK_REGION )
			{
				...
				for( int j=0; j<INT(pRECT->m_vPOLY.size()); j++)
				{
		...
		for( int i = 0; i<INT(pTILE->size()); i++)
		{
			...
			for( int j=0; j<INT(vBLEND[i].size()); j++)
			{
				...
				for( int k=0; k<INT(pRECT->m_vPOLY.size()); k++)
				{
									...
									int nCellZ)
{
	...
	for( int i =1; i<INT(m_vDIFFUSELOCK.size()); i++)
