...
:CTClientUIBase( pParent, pDesc)
{
	...
	{
		CPoint point;
		LP_FRAMEDESC pGAUGE = pParser->FindFrameTemplate( ID_FRAME_CHATNEW );
...
HRESULT CTMainUI::Render( DWORD dwTickCount)
{
	if(IsVisible())
	{
		...
		if(m_bNotifyNewCashCabinet)
			...
			{
				TComponent* pBar = pExpG->FindKid(pExpG->m_pDESC->m_vCOMP.m_vEX.gauge.bar[0]);
				CRect rcBar;
			...
			{
				TComponent* pBar = pLotG->FindKid(pLotG->m_pDESC->m_vCOMP.m_vEX.gauge.bar[0]);
				CRect rcBar;
				pBar->GetComponentRect( &rcBar );
				CSize size = rcBar.Size();
				CRect rcSrc = CRect( 0, 0, size.cx, size.cy );				
