...
void CTOptionItemDetInfo::_ConvertAndSetTitle(const CString& strSource, DWORD dwColor)
{
	...
	if(m_pItemInst->GetGrade() > 0)
	{
		...
		{
			if(m_pItemInst->GetGradeEffect() == IE_SEA)
				m_TitleB.m_strText += " " + CTChart::LoadString( TSTR_SEA );
			else if(m_pItemInst->GetGradeEffect() == IE_FIRE)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_FIRE );
			else if(m_pItemInst->GetGradeEffect() == IE_LIGHTING)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_LIGHTNING );
			else if(m_pItemInst->GetGradeEffect() == IE_ICE)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_ICE );
			else if(m_pItemInst->GetGradeEffect() == IE_BLACK)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_BLACK );
			else if(m_pItemInst->GetGradeEffect() == IE_PINK)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_PINK );
			else if(m_pItemInst->GetGradeEffect() == IE_STORMBLUE)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_STORMBLUE );
			else if(m_pItemInst->GetGradeEffect() == IE_POISENGREEN)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_POISENGREEN );
			else if(m_pItemInst->GetGradeEffect() == IE_MAGMA)
				m_TitleB.m_strText += " " +  CTChart::LoadString( TSTR_MAGMA );
		}
	}
