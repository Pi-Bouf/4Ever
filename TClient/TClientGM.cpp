...
int CTClientGame::OnGM_SELECT_TEXTURE_DETAIL()
{
	...
	for( int i=0; i<TEXTURE_DETAIL_COUNT; ++i )
...
int CTClientGame::OnGM_TOGGLE_CASH_SHOP()
{
	...
	ShellExecute(NULL, _T("open"), "http://shop.4story.pw",NULL,NULL,SW_SHOWDEFAULT);
	return TERR_NONE;
}
...
int CTClientGame::OnGM_TOGGLE_TITLE()
{
	...
	if( m_vTFRAME[TFRAME_TITLE]->IsVisible() )
		DisableUI(TFRAME_TITLE);
	else
...
int CTClientGame::OnGM_LEVEL1()
{
	...
	for( BYTE i=0; i<MAX_SLOT; i++)
	{
		...
		if(bEMPTY)
		{
			...
			m_pMainWnd->m_pTGROUP->m_bGroupID,
			strNAME, i,
