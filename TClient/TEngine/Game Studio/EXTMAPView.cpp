...
BOOL CEXTMAPView::DoDETAIL( FLOAT fPosX, FLOAT fPosZ, BOOL bStart)
{
	...
	WORD wCount = (WORD) pDoc->m_pItem->m_vDETAIL.size();
			...
			while( nPosZ < nEndZ )
			{
				...
				while( nPosX < nEndX )
				{
					if(rect.PtInRect(CPoint( nPosX, nPosZ)))
					{
						...
						if( fRadius <= ((FLOAT) pBar->m_nSize) / 2.0f )
						{
							BYTE size = (BYTE)pDoc->m_UNIT[m_bUNIT[i]].m_vDETAIL.size();
							BYTE j = 0;
							BOOL bFound = FALSE;
							for( ; j < size; ++j )
							{
								WORD wFind = pDoc->m_UNIT[m_bUNIT[i]].m_vDETAIL[j];
								if( wDETAIL == wFind )
								{
									bFound = TRUE;
									pDoc->m_UNIT[m_bUNIT[i]].m_pDETAIL[nIndex] = j;
									break;
								}
							}
							if( !bFound )
							{
								pDoc->m_UNIT[m_bUNIT[i]].m_vDETAIL.push_back( wDETAIL );
								pDoc->m_UNIT[m_bUNIT[i]].m_pDETAIL[nIndex] = size;
							}
						}
					}
