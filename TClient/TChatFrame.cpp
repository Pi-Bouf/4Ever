	  ...
	  m_dwChatTargetID(0), m_pChatEditBack(NULL), m_pChatTabBack(NULL), m_dwNotifyTime(0)
{
	...
	LP_FRAMEDESC pSizeBarFrameDesc = CTClientGame::GetInstance()->m_pTParser->FindFrameTemplate( ID_FRAME_CHATNEW );
	...
	LP_FRAMEDESC pChatMenuDesc = CTClientGame::GetInstance()->m_pTParser->FindFrameTemplate( ID_FRAME_CHATMENU );
	...
	for( INT i=0; i<TCHAT_SET_COUNT; ++i)
	{
		...
		CTChatTabButton* m_vTabs = new CTChatTabButton(this, pTabTemplate->m_pDESC);
		m_vTabs->m_id = GetUniqueID(NULL);
		
		delete pTabTemplate;

		CTChatList*	m_vList = new CTChatList(this, m_pListTemplate->m_pDESC, pParent);
	...
	LP_FRAMEDESC pFrameDESC = pFrame->m_pDESC;

	m_pNotifyList = new CTChatList(this, pListTemplate2->m_pDESC, pParent);
...
void CTChatFrame::ResetSetting()
{
	...
	str.Format( _T(REG_SET_FRAMENUM), i);
...
void CTChatFrame::AddList(DWORD dwChatFlag)
{
...

	{
		CPoint cp;
		TComponent* pTabTemplate;
		pTabTemplate = pSizeBarFrame->FindKid(26324);
		pSizeBarFrame->RemoveKid( pTabTemplate );
		...
		CTChatList*	m_vList = new CTChatList(this, m_pListTemplate->m_pDESC, m_pParent);
