...
#ifdef NEW_IF
	SetRegistryKey(_T("4StoryPW_NEWUI"));
#else
	SetRegistryKey(_T("4StoryPW"));
#endif
...
BYTE CTClientApp::MainProc()
{
	...
	/*if(CWnd::GetForegroundWindow() == NULL)
	{
...
void LogFileWrite(const char *pszParam, ...)
{
	...
	sprintf( strFile, "C:\\log4s\\Client_%04d%02d%02d.log",nowTime.GetYear(), nowTime.GetMonth(), nowTime.GetDay() );
