	...
	for( i=0; i<nCount; i++)
	{
		...
		*((LPDWORD) pDATA) = dwLength;
		...
		if( dwLength > 0 )
		{
			...
			//{
			//	BYTE bDETAIL = *((LPBYTE) pDATA);
			//	WORD wDETAIL = bDETAIL;

			//	if( bDETAIL == 0xFF )
			//		wDETAIL = 0xFFFF;
			//	else if( bDETAIL & 0x80 )
			//		wDETAIL = (bDETAIL & 0x7F) | 0x8000;

			//	MAPWORD::iterator finder = mapDETAIL.find( wDETAIL );
			//	if( finder == mapDETAIL.end() )
			//	{
			//		mapDETAIL.insert( make_pair< WORD, WORD >( wDETAIL, bIndex ) );
			//		vecDETAIL.push_back( wDETAIL );
			//		*((LPBYTE) pDATA) = bIndex;
			//		bIndex++;
			//	}
			//	else
			//	{
			//		*((LPBYTE) pDATA) = (BYTE)(*finder).second;
			//	}

			//	pDATA += sizeof(BYTE);
			//}

			//m_mapDETAIL.insert( make_pair< DWORD, VECTORWORD >( i, vecDETAIL ) );
		}
		//----- pDETAIL.
...
void CEXTMAPItem::Save( CFile* pFile)
{
	...
	for( i=0; i<nCount; i++)
	{
		...
		/*-------- Converte.
		VECTORWORD& vecDETAIL = m_mapDETAIL[i];
		DWORD size = vecDETAIL.size();
		dwLength = size * sizeof( WORD );
		pFile->Write( &dwLength, sizeof( DWORD ) );
		for( DWORD j = 0; j < size; ++j )
		{
			pFile->Write( &vecDETAIL[j], sizeof( WORD ) );
		}
		/*/
							...
							DWORD dwIndex)
{
	...
	if(pVDETAIL && dwCount > 0)
	{
		...
		DWORD dwSize = dwCount / sizeof(WORD);
							  ...
							  DWORD dwIndex)
{
	...
	if(pVDETAIL)
	{
		DWORD dwCount = DWORD(pVDETAIL->size()) * sizeof(WORD);

		file.Write( &dwCount, sizeof(DWORD));
		for( int i = 0; i < pVDETAIL->size(); ++i )
		{
			WORD wDETAIL = (*pVDETAIL)[i];
			file.Write( &wDETAIL, sizeof(WORD) );
		}
	}
	else
	{
...
void CEXTMAPItem::BuildTILETEX( int nPosX, int nPosZ)
{
	...
	memset( pDETAIL, 0xFF, m_HugeMAP.m_nUnitLength * m_HugeMAP.m_nUnitLength * sizeof(BYTE));
	LoadUNIT( NULL, NULL, NULL, NULL, NULL, &vDETAIL, pDETAIL, NULL, nIndex);
	...
	for( int i=0; i<m_HugeMAP.m_nUnitLength; i++)
		for( int j=0; j<m_HugeMAP.m_nUnitLength; j++)
		{
			...
			FLOAT fSCALE = 32.0f;
			WORD wDETAIL = TILE_NULL;
			if( bDETAIL != 0xFF )
			{
				wDETAIL = vDETAIL[ bDETAIL ];
				fSCALE = wDETAIL == TILE_NULL ? 32.0f : m_vDETAILS[ wDETAIL ];
			}
			...
			CTextureItem *pTEX = wDETAIL == TILE_NULL ? NULL : GetTextureItem(m_vDETAIL[wDETAIL]);
	...
	UpdateUNIT( NULL, NULL, pTILE, NULL, NULL, NULL, NULL, NULL, nIndex);
	pTRENDER->Release();
	pBACK->Release();
	pZBUF->Release();
	pTILE->Release();
...
void CEXTMAPItem::RegulateHEIGHT( int nPosX, int nPosZ)
{
	...
	LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pHEIGHT, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX);
	if( nPosX > 0 && nPosZ > 0 )
	{
		LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pH, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pH[(m_HugeMAP.m_nUnitLength + 1) * (m_HugeMAP.m_nUnitLength + 1) - 1] = pHEIGHT[0];
		UpdateUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pH, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
	}

	if( nPosZ > 0 )
	{
		LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pH, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
		memcpy( &pH[m_HugeMAP.m_nUnitLength * (m_HugeMAP.m_nUnitLength + 1)], pHEIGHT, (m_HugeMAP.m_nUnitLength + 1) * sizeof(FLOAT));
		UpdateUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pH, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
	}

	if( nPosX > 0 )
	{
		LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL,NULL, pH, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		for( int i=0; i<m_HugeMAP.m_nUnitLength + 1; i++)
			pH[(i + 1) * (m_HugeMAP.m_nUnitLength + 1) - 1] = pHEIGHT[i * (m_HugeMAP.m_nUnitLength + 1)];
		UpdateUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pH, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
	}
								  ...
								  int nUnitID)
{
	...
	LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, extmap.m_pDATA, nUnitID);
	...
	for( itATTR = pOBJ->m_mapATTR.begin(); itATTR != pOBJ->m_mapATTR.end(); itATTR++)
	{
			...
			(*itSTR).second == CString("ID_OBJ_TYPE") )
		{
			...
			if( pATTR && pATTR->m_bValueType == ATTRTYPE_INT && (
				*((int *) (*itATTR).second->m_pValue) == 5 ||	// OT_WATER
				*((int *) (*itATTR).second->m_pValue) == 6 ))	// OT_COLLISION
				return FALSE;
...
void CEXTMAPItem::BuildSHADE( int nPosX, int nPosZ)
{
	...
	LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pCENTER, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX);
	if( nPosX > 0 && nPosZ > 0 )
	{
		LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pSIDE, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
	...
	if( nPosZ > 0 )
	{
		LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pSIDE, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
	...
	if( nPosX > 0 )
	{
		LoadUNIT( NULL, NULL, NULL, NULL, NULL, NULL, NULL, pSIDE, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
	...
	UpdateUNIT( pIMG, NULL, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX);
	pIMG->Release();
...
void CEXTMAPItem::RegulateSHADE( int nPosX, int nPosZ)
{
	...
	LoadUNIT( &pSHADE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX);
	...
	if( nPosX > 0 && nPosZ > 0 )
	{
		...
		LoadUNIT( &pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		...
		pSIDE->UnlockRect();
		UpdateUNIT( pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->Release();
		pSIDE = NULL;
	}
	...
	if( nPosZ > 0 )
	{
		...
		LoadUNIT( &pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
		pSIDE->LockRect( &lrSIDE, NULL, 0);
		...
		pSIDE->UnlockRect();
		UpdateUNIT( pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
		pSIDE->Release();
		pSIDE = NULL;
	}
	...
	if( nPosX > 0 )
	{
		...
		LoadUNIT( &pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->LockRect( &lrSIDE, NULL, 0);
		...
		pSIDE->UnlockRect();
		UpdateUNIT( pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->Release();
		pSIDE = NULL;
	}
							 ...
							 int nPosZ)
{
	...
	UpdateUNIT( NULL, pIMG, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX);
	pIMG->Release();
...
void CEXTMAPItem::RegulateSKIN( int nPosX, int nPosZ)
{
	...
	LoadUNIT( NULL, &pSKIN, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX);
	...
	if( nPosX > 0 && nPosZ > 0 )
	{
		...
		LoadUNIT( NULL, &pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->LockRect( &lrSIDE, NULL, 0);
		...
		pSIDE->UnlockRect();
		UpdateUNIT( NULL, pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->Release();
		pSIDE = NULL;
	}
	...
	if( nPosZ > 0 )
	{
		...
		LoadUNIT( NULL, &pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
		pSIDE->LockRect( &lrSIDE, NULL, 0);
		...
		pSIDE->UnlockRect();
		UpdateUNIT( NULL, pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, (nPosZ - 1) * m_HugeMAP.m_nUnitCountX + nPosX);
		pSIDE->Release();
		pSIDE = NULL;
	}
	...
	if( nPosX > 0 )
	{
		...
		LoadUNIT( NULL, &pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->LockRect( &lrSIDE, NULL, 0);
		...
		pSIDE->UnlockRect();
		UpdateUNIT( NULL, pSIDE, NULL, NULL, NULL, NULL, NULL, NULL, nPosZ * m_HugeMAP.m_nUnitCountX + nPosX - 1);
		pSIDE->Release();
		pSIDE = NULL;
	}
							  ...
							  LPVECTORDWORD pVUNIT)
{
	...
	for( int i=0; i<nCount; i++)
	{
		VECTORWORD vDETAIL;
		vDETAIL.clear();
		memset( pDETAIL, 0x00, dwSIZE * sizeof(BYTE));
		LoadUNIT(
			NULL,
			NULL,
			NULL,
		...
		if( vDETAIL.size() == 0 )
			vDETAIL.push_back( TILE_NULL );

		for( DWORD j=0; j<vDETAIL.size(); j++)
		{
			WORD wTILE = vDETAIL[j] & TILE_MASK;

			if( vDETAIL[j] != TILE_NULL && pINDEX->find(wTILE) == pINDEX->end() && m_vDETAIL[wTILE] )
				pINDEX->insert( MAPDWORD::value_type( wTILE, TILE_NULL));
		}
	}
...
void CEXTMAPItem::ResetDETAIL( LPMAPDWORD pINDEX,
							   ...
							   DWORD dwCOUNT)
{
	if( !pINDEX || !pVDETAIL || !pDETAIL )
		return;

	//for( DWORD i=0; i<dwCOUNT; i++)
	//{
	//	WORD wTILE = (*pVDETAIL)[ pDETAIL[i] ] & TILE_MASK;
	//	MAPDWORD::iterator finder = pINDEX->find(wTILE);

	//	if( finder != pINDEX->end() )
	//	{
	//		wTILE = (WORD) (*finder).second;

	//		if((*pVDETAIL)[ pDETAIL[i] ] & 0x8000)
	//			wTILE |= 0x8000;

	//		(*pVDETAIL)[ pDETAIL[i] ] = wTILE;
	//	}
	//	else
	//		(*pVDETAIL)[ pDETAIL[i] ] = TILE_NULL;
	//}

	int nSize = (int)pVDETAIL->size();
	for( int i=0; i<nSize; ++i )
	{
		WORD wLocalINDEX = (*pVDETAIL)[i] & TILE_MASK;
		MAPDWORD::iterator finder = pINDEX->find( wLocalINDEX );

		if( finder != pINDEX->end() )
		{
			WORD wGlobalINDEX = (WORD)(*finder).second;

			if( wLocalINDEX & 0x8000 )
				wGlobalINDEX |= 0x8000;

			(*pVDETAIL)[i] = wGlobalINDEX;
		}
	}
}
...
void CEXTMAPItem::ExpGEO( LPBMPARAM lParam)
{
	...
	for( i=0; i<dwCount; i++)
	{
		...
		if(bEnable)
		{
			...
			// DETAIL INDEX.
			ResetDETAIL( &mapINDEX, &vDETAIL_INDEX, pDETAIL, m_HugeMAP.m_nUnitLength * m_HugeMAP.m_nUnitLength);
			dwLength = (DWORD)vDETAIL_INDEX.size() * sizeof(WORD);
			file.Write( &dwLength, sizeof(DWORD));
			for( int j=0; j<vDETAIL_INDEX.size(); ++j )
			{
                WORD wDETAIL = vDETAIL_INDEX[j];
				file.Write( &wDETAIL, sizeof(WORD) );
			}
			// DETAIL INDEX.
		}
...
void CEXTMAPItem::ImpGEO( LPBMPARAM lParam)
{
	...
	for( i=0; i<dwCount; i++)
	{
		...
		if(dwDETAIL)
		{
			...
			if( finder == mapTILE.end() )
			{
				wTILE = WORD(m_vDETAIL.size());
				mapTILE.insert( MAPDWORD::value_type( dwDETAIL, DWORD(wTILE)));
		...
		if( mapINDEX.find(i) == mapINDEX.end() )
			mapINDEX.insert( MAPDWORD::value_type( i, DWORD(wTILE)));
	}
	...
	for( i=0; i<dwCount; i++)
	{
		...
		if(bEnable)
		{
			...
			VECTORWORD vecDETAIL;
			file.Read( &dwLength, sizeof(DWORD) );
			DWORD dwSize = dwLength / sizeof( WORD );
			for( int j=0; j<dwSize; ++j )
			{
				WORD wDATA = 0;
				file.Read( &wDATA, sizeof(WORD) );
				vecDETAIL.push_back( wDATA );
			}
			// DETAIL INDEX.
			...
			if(vUNIT.IntersectRect( &vUNIT, &m_vFROM))
			{
				vUNIT.OffsetRect(
					m_vTO.x - m_vFROM.left,
					m_vTO.y - m_vFROM.top);
				ResetDETAIL( &mapINDEX, &vecDETAIL, pEXTDETAIL, dwUNIT * dwUNIT);
