...
DWORD CTPatchSvrModule::OnCT_CHANGEIF_REQ(CSqlDatabase * pDB, CTPatchSession * pSession)
{
	BYTE bOption;
	pSession->m_Recv
		>> bOption;
	...
	DEFINE_QUERY(pDB, CTBLInterface)
	...
	CString strFTP = m_strFTP + "/interface";
	pSession->SendCT_NEWPATCH_ACK(strFTP, &m_addrLOGIN, &vPatch, NULL);
