...
/*void CTChart::InitTSKILLTEMP( CString strPath )
{
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		>> pTSKILL->m_wActSkillID        ///RERüLs°r?sLl
		>> pTSKILL->m_wNeedItemID        ///LN¸dlCRELU
		>> pTSKILL->m_wDefSkillID        ///´ë»óRE°É¸°aöÇÁ
		>> pTSKILL->m_dwClassID         ///Á÷l÷ÄÚµl
		>> pTSKILL->m_bKind          ///CZLs±¸sÐ
		>> pTSKILL->m_bPositive         ///RE·Îzn ÇR·Îzn
		>> pTSKILL->m_bLevel         ///zä±¸·as§
		>> pTSKILL->m_bLevelInc         ///zä±¸·as§Áo°?
		>> pTSKILL->m_bMaxLevel         ///AÖ´ë?sLl·as§
		>> pTSKILL->m_fPrice         ///°?°ÝsnR?
		>> pTSKILL->m_bDuraSlot         ///l»±¸°¨LN??·Ô(Aß°?)
		>> pTSKILL->m_dwWeaponID        ///»çzëa«±â
		>> pTSKILL->m_wUseHP         ///HPLN¸dsnR?
		>> pTSKILL->m_wUseMP         ///MPLN¸dsnR?
		>> pTSKILL->m_bHitInit         ///Ls°rR?AE±â°S(Aß°?)
		>> pTSKILL->m_bHitInc         ///Ls°rR?Áo°?°S(Aß°?)
		>> pTSKILL->m_dwSpellTick        ///aßµz?A°L
		>> pTSKILL->m_bIsRide         ///aöÇÁÁß L» Lö lr´Â ?sLl
		>> pTSKILL->m_dwDelay         ///Rç»çzë´ë±â
		>> pTSKILL->m_nDelayInc         ///Rç»çzë ´ë±â Áo°¨snR?
		>> pTSKILL->m_dwGroupTick        ///°cz­´ë±â
		>> pTSKILL->m_dwInterval        ///aÝsa?A°L
		>> pTSKILL->m_bDelayType        ///°rLÓRuzë
		>> pTSKILL->m_wModeID         ///LÂLL
		>> pTSKILL->m_bTargetType        ///»çzë´ë»ó ÇÇlC
		>> pTSKILL->m_bRangeType        ///´ë»óCÇs°
		>> pTSKILL->m_fMinRange         ///AÖLN»çÁ¤°L¸®
		>> pTSKILL->m_fMaxRange         ///AÖ´ë»çÁ¤±â¸®
		>> pTSKILL->m_fAtkRange         ///RÎ?Ä Cr,aÝ°c
		>> pTSKILL->m_fBufRange         ///zR¶óaüR§
		>> pTSKILL->m_dwDuration        ///Ruzë?A°LAE±â°S
		>> pTSKILL->m_dwDurationInc        ///Ruzë?A°LÁo°?snR? 
		>> pTSKILL->m_bCanCancel        ///»çzëRÚ AëLN °?´É z©sÎ(Aß°?)
		>> pTSKILL->m_bContinue         ///µzRUÁöLÓz©sÎ
		>> pTSKILL->m_wIconID         ///lCREÄÜID
		>> pTSKILL->m_wItemID[0]        ///aß»çlCRELU1
		>> pTSKILL->m_wItemID[1]        ///aß»çlCRELU2
		>> pTSKILL->m_bActive         ///CÐ?Ase=0 l×CLse=1
		>> pTSKILL->m_bLoop          ///RÚµzaÝsaz©sÎ
		>> pTSKILL->m_bActionID[TSKILLACTION_SPELL]    ///ÁÖa®µzRU
		>> pTSKILL->m_bActionID[TSKILLACTION_ACTIVATE]   ///aßµzµzRU
		>> pTSKILL->m_bActionID[TSKILLACTION_MOVEACTIVATE]  ///REµzaßµzµzRU
		>> pTSKILL->m_bActionID[TSKILLACTION_DEFEND]   ///aclîµzRU
		>> pTSKILL->m_dwInfoID         ///LrCÁID
		>> pTSKILL->m_dwSFX[TSKILLSFX_SPELL]     ///?ARüREClC®
		...
		>> pTSKILL->m_dwSFX[TSKILLSFX_END]      ///ÇRÁ¦REClC®
		>> pTSKILL->m_dwSFX[TSKILLSFX_ARROW]     ///aß»çALREClC®
		>> pTSKILL->m_bShowIcon         ///lCREÄÜs¸RÓ
		>> pTSKILL->m_bShowTime         ///l?Rs?A°Ls¸RÓ
		>> pTSKILL->m_bShowCritical        ///L©¸®CLÄARuzë
		>> pTSKILL->m_bUseInHold        ///°¤Cu »óLÂz?L­ »çzë °?´Éz©sÎ
		>> pTSKILL->m_bStaticWhenDie       ///»ç¸Á?A aöÇÁ RZÁö
		>> pTSKILL->m_fMoveDist        ///Lr°LREµz °L¸®
		>> unused
		>> unused
		>> unused
		>> unused
